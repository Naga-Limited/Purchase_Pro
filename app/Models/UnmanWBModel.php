 ...
 public function getVehicleDetails($data){
    ...
    $wBuilder = $this->db->table("weighment_info")
        ->select("
            ...
            master_module.moduleType as moduleTypes
        ")
    ...
    if (!empty($weightResult)) {
		...
		if ($firstWeight > 0 && $currentWeight > 0) {
		    if ($movementType === 1) {
		       ...
		       if ($currentWeight <= $firstWeight && $w['userGateId'] != 17) {
		            throw new \Exception("Loading: second weight must be greater than first weight: {$currentWeight}");
		        }else if($currentWeight < $firstWeight && $w['userGateId'] == 17){
		            throw new \Exception("Loading: second weight must be greater than first weight: {$currentWeight}");
		        }
        ...
        if (($firstWeight && $secondWeight ) && in_array($moduleType, [5,7,13,21,33,19,29,39])) {
            ...
            $query = $this->db->table("gate_in_out_info_details")
				->where('gateInOutInfoId',$gateId);
			$count = $query->countAllResults();
            if($count == 0 && in_array($moduleType, [29])){
                ...
                return ['status'=>'success','message'=>'Chained Weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'Chanined','NetWeight'=>$netWeight1 ,'vaNumber'=>$vaNumber,'module'=>$w['moduleTypes']];    
            }  
        ...
        if ($firstWeight && empty($secondWeight)) {
            ...
            if (($w['waitingAt'] ?? null) == 9) {
                ...
                return ['status'=>'success','message'=>'Second weight check added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'Second Weight Check','vaNumber'=>$vaNumber,'module'=>$w['moduleTypes']];
            }
            ...
            if (in_array($moduleType, [1,2,43]) && $movementType === 1) {
                ...
                if($w['subModuleTypeId'] == 20 || $w['subModuleTypeId'] == 21 || $w['subModuleTypeId'] == 19){
                    $TYPE = 'Other-Sales';
                    $tripSheetNumber = $w['tripSheetNumber'] ?? 'No Tripsheet';
                }else{
                ...
                return ['status'=>'success','message'=>'Second weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'Second','NetWeight'=>$netWeight,'vaNumber'=>$vaNumber,'module'=>$w['moduleTypes']];
            }
            ...
            if($movementType === 2 && in_array($moduleType, [5,20,6,13,2])){
                $this->db->transStart();
                $this->db->table("weighment_info")
                    ->where('id', $w['id'])
                    ->update([
    ...
    $firstWt = $this->db->table("gate_in_out_info")
        ...
        ->groupStart()
            ...
            ->orGroupStart()
                ...
                ->whereIn('gate_in_out_info.moduleType', [12,15,21,33,34,5,6,45])
            ->groupEnd()
        ->groupEnd()
    ...
    if (!empty($firstWtResult)) {
        ...
        $subModuleTypeId = $f['subModuleTypeId'] ?? null;
        ...
        if ($waitingAt == 2 && in_array($moduleType, [1,2,43])) {
            $TYPE1 = 'FG-STO';
            if(in_array($subModuleTypeId, [19,20,21])){
                $TYPE1 = 'Other-Sales';
                $tripSheetNumber = $tripSheetNumber ?? 'No Tripsheet';
            }else{
                $TYPE1 = 'FG-STO';
            ...
            $sap_data = [
                ...
                "ZZVA_NO" => $f['vaNumber'] ?? '',
                "ZZDRIVER_PH" => $f['driverMobileNumber'] ?? '',
                "ZZROUTE" => $f['route'] ?? '',
                ...
                "ZZSecondWeight" => '',
                "ZZNetweight" => '',
                "ZZSHIPMENT_NO" => $fgDetailsData[0]->SAP_LINE[0]->SHIPMENTORDERNO ?? '',
	    ...
	    //print_r($sap_data);exit;
            ...
            $this->db->table("weighment_info")->insert([
                ...
                // 'netWeight' => '',
                'createdOn' => $CurrentDateTime,
                'firstWtSystemNo' => $data->systemNo ?? $master['systemNo'],
                'createdBy' => $master['userId'] ?? 0,
                'gateInOutInfoId' => $gateId
            ]);
            $this->db->table("gate_in_out_info")
            ...
            return ['status'=>'success','message' => 'First weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'First','vaNumber'=>$vaNumber,'module'=>$f['moduleTypes']];
        }
...
if (!empty($testResult)) {
    ...
    if (!empty($evRes)) {
        if (!isset($data->weight) || $data->weight < 1000) {
            throw new \Exception("Invalid weight. Please check vehicle: {$data->vehicle}");
        }
        $ev = $evRes[0];
        ...
        if ($vehicleStatus === 23 && $firstW == 0) {
            ...
            return ['status'=>'success','message' => 'First weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'First','vaNumber'=>$vaNumber,'module'  => $ev['SCREEN_TYPE'] == 'EVADP' ? 'IAS' : 'Silo to Mill'];
        }
        if ($vehicleStatus === 23 && $firstW > 0) {
            $this->db->transStart();
        ...
        if ($vehicleStatus === 24 && $firstW > 0) {
            ...
            return ['status'=>'success','message'=>'Second weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'Second','NetWeight'=>$net,'vaNumber'=>$vaNumber,'module'  => 'IAS'];
            ...
            return ['status'=>'success','message'=>'Second weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'Second','NetWeight'=>$net,'vaNumber'=>$vaNumber,'module'  => 'Silo to Mill'];
        }
    }
	...
	$distinctArrivalIds = array_unique(array_column($pRes, 'TRUCK_NO'));
        ...
        if ($vehicleStatus === 23 && $firstW == 0) {
            ...
            return ['status'=>'success','message' => 'First weight added for vehicle ' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'First','vaNumber'=>$vaNumber,'module'  => $MODULE];
        }
        if ($vehicleStatus === 23 && $firstW > 0) {
            // first weight
            $this->db->transStart();
        ...
        if ($vehicleStatus === 24 && $firstW > 0) {
            ...
            $this->db->transComplete();
            $MODULE = '';
            if ($screenType == 'SILOTOMILL') {$MODULE = 'Silo To Mill';}
            elseif ($screenType == 'IAS') {$MODULE = 'IAS';}
            elseif (strtolower($vehicleType) == 'rake') {$MODULE = 'RM Wheat';}
            else {$MODULE = 'RM Wheat';}
            if ($this->db->transStatus() === false) throw new \Exception("DB failed for purchase_info second weight");
            return ['status'=>'success','message'=>'Second weight added for vehicle' . $data->vehicle . 
                 ' (Weight: ' . $currentWeight . ' kg)','Type'=>'Second','NetWeight'=>$net,'vaNumber'=>$vaNumber,'module'  => $MODULE];
        }
    }
