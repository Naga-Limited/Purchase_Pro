        ...
        $builder = $this->db->table("purchase_order")
            ->select("
                ...
                gate_in_out_info.gateInDateStamp,
                 CASE
                    WHEN master_plant.isLotStatus = 1
                    AND po_type.isLotStatus = 1
                    THEN 1
                    ELSE 0
                END AS isLotStatus
            ", false)
...
public function ReceiptDetailReport($fromDate, $toDate, $moduleTypeId, $userInfoId) {
    ...
    $builder = $this->db->table("receipt_material_info")
        ->select("
            ...
            DATE_FORMAT(receipt_material_info.lotUpdationAt, '%d-%m-%Y %H:%i:%s') AS lotUpdationAt,
            DATE_FORMAT(receipt_material_info.qcCheckAt, '%d-%m-%Y %H:%i:%s') AS qcCheckAt,
            ...
            ) AS duration,IF(receipt_material_info.invoiceQty > 0, receipt_material_info.invoiceQty, receipt_material_info.receivedQty) AS invoiceQty,po_type.isLotStatus
        ")
