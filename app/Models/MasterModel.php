...
public function getPlant($Id) {
 $fetchsql = "SELECT a.`ID`, `WERKS`, `PLANT_NAME`,`OwnWB`,`plant_subdivision`,`isMovement`, concat(a.`WH_CODE`, '-', b.`WH_NAME`) as WH_NAME, a.`WH_CODE`, a.`InsBy`, a.`InsDt`, a.`ModBy`, a.`ModDt`, a.`RecStatus` FROM `master_plant` a, `warehouse_master` b WHERE a.WH_CODE=b.WH_CODE and a.RecStatus='1' and a.ID='$Id'";
 $builder =  $this->db->query($fetchsql);
 return  $builder->getResultArray();
}
...
//master_vendor
 public function updatemaster_vendor($id,$Data){
  if($this->getMastervendorDuplicateChk($Data->Code, $Data->Category,$id)=="0")
{
...
return $InsId;
 ...
 public function  insertmaster_vendor($Data){
  // print_r($Data);exit;
  if($this->getMastervendorDuplicateChk($Data->Code,$Data->Category)=="0")
  {
...
public function getMaster_vendorDetailsById($Id) {
 $fetchsql = "SELECT `Id`, `Code`, `Name`, `Category`,`plant_code`, `BagCuttingCharge`, `RelottingCharge`, `InsBy`, `InsDt`, `ModBy`, if(`ModDt` is null,'',ModDt) as ModDt, `RecStatus`,`tds_code`,`tds_name` FROM `master_vendor` WHERE RecStatus='1' and Id='$Id'";
 $builder =  $this->db->query($fetchsql);
 return  $builder->getResultArray();
}
...
//master_vendor
public function getMastervendorDuplicateChk($code, $Category, $id = "") {
    $cnd = "";

    if ($id != "") {
        $cnd .= " AND Id <> '" . $id . "'";
    }
    $fetchsql = "SELECT Id FROM master_vendor WHERE Code = '$code' AND Category = '$Category' " . $cnd;
    $builder = $this->db->query($fetchsql);
    return count($builder->getResultArray());
}
