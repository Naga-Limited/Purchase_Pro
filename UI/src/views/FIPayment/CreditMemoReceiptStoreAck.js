...
function CreditMemoReceiptStoreAck() {
    ...
    const [rejectMemoId, setRejectMemoId] = useState(null);
    ...
    const openDocument = (row, docKey, url) => {
        ...
        setViewedDocs((prev) => ({
            ...prev,
            [row.credit_memo_id]: { ...prev[row.credit_memo_id], [docKey]: true },
        }));
    };

    const canActOnRow = (row) => {
        const viewed = viewedDocs[row.credit_memo_id] || {};
    ...
    const handleApprove = async (row) => {
        const confirmed = await confirmDialog({
            title: 'Acknowledge this credit memo?',
            confirmText: 'Book Keeping',
            cancelText: 'Cancel',
        });
        if (confirmed) updateApprovalStatus(row.credit_memo_id, APPROVAL_STATUS.ACKNOWLEDGED);
    };

    const openRejectModal = (row) => {
        setRejectMemoId(row.credit_memo_id);
    ...
    const columns = [
        ...
        {
            name: 'Credit memo Copy',
            minWidth: '130px',
            cell: (row) => (
                <Button color={viewedDocs[row.credit_memo_id]?.invoice ? 'success' : 'light'} size="sm"
        ...
        },
        {
            ...
            minWidth: '150px',
            cell: (row) => (
                <Button color={viewedDocs[row.credit_memo_id]?.backPaper ? 'success' : 'light'} size="sm"
        ...
        },
        {
            ...
            minWidth: '260px',
            cell: (row) => {
                const disabled = submittingId === row.credit_memo_id || !canActOnRow(row);
    ...
    return (
        <CardComponent header="Credit Memo Store Acknowledge List">
            ...
            <Modal show={rejectModalOpen} onHide={closeRejectModal} centered>
                ...
                <Modal.Footer style={{ background: '#f8f9fa' }}>
                    ...
                    <Button color="danger" size="sm" disabled={submittingId === rejectMemoId} onClick={handleRejectSubmit}>
                        Reject Credit Memo
                    </Button>
                </Modal.Footer>
            </Modal>
...
export default CreditMemoReceiptStoreAck;
