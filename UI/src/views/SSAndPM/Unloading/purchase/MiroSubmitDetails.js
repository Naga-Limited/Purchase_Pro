...
const MiroSubmitDetails = ({ setShow, show, gateInOutInfoId,lines,status,miroIds,getLoadingData,weighment }) => {
                ...
                if (type == 1 && data.success == true) {
                    ...
                    confirmDialog({
                        ...
                        if (data.results == 3 || data.results == 0) {
                            ...
                            setSimulateData([]);
                            setShow3(false);  // Reloads the page after the confirm dialog is closed
                        }else {
                            setShow(true);
                            setActualStatus(data.results);
                        }
    ...
    const docDate = lines[0]?.docDate;

    if (!docDate || !lines[0]?.paymentMethod) return "";

    // Remove first character and convert to number
    const days = parseInt(lines[0].paymentMethod.substring(1), 10);
        ...
        <div>
            <Modal show={show} centered size="xl">
                ...
                <Row>
                        ...
                        <Col md="3" sm="3" >
                            ...
                            <Input
                                type="date"
                                value={invoiceDate} // Auto show current date
                                min={date_control}
                                ...
                                onChange={(e) => {
                                    ...
                                    handleInputChange(e.target.value, 'posting');
                                }}
                            />
                        </Col>}
                        {(lines[0]?.status == 2 || lines[0]?.status == 4) &&
           ...
           <Modal
            ...
            >
                <Card>
                <Modal.Header style={{
                backgroundColor: "#7367f0"}} closeButton>
                <Modal.Title 
                    style={{
                        color: "white",
                        textAlign: "center",
                ...
                <Modal.Body>
                 <Row>
                    ...
                    <Col md="3" sm="3">
                        <FormGroup>
                            <Label className="font-weight-bold d-block" style={{ textAlign: "left" }}>Posting Date</Label>
                            <Input type="date" placeholder="Invoice Date" value={invoiceDate} disabled />
                        </FormGroup>
                    </Col>
                     <Col md="3" sm="3">
                        <FormGroup>
                    ...
                    <Col md="3" sm="3">
                        <FormGroup>
                            <Label className="font-weight-bold d-block" style={{ textAlign: "left" }}>TDS</Label>
                            <Input type="text" placeholder="TDS" value={form?.values?.TDS?.value} disabled />
                        </FormGroup>
                    </Col>
                    <Col md="3" sm="3">
                        <FormGroup>
                            <Label className="font-weight-bold d-block" style={{ textAlign: "left" }}>Base Line Date</Label>
                            <Input type="date" placeholder="Invoice Date" value={lines[0]?.docDate} disabled />
                        </FormGroup>
                    </Col>
                    <Col md="3" sm="3">
                        <FormGroup>
                            <Label className="font-weight-bold d-block" style={{ textAlign: "left" }}>
                                Due Date
                            </Label>
                            <Input
                                type="text"
                                value={
                                    lines[0]?.docDate && lines[0]?.paymentMethod
                                        ? (() => {
                                            const days = parseInt(
                                                lines[0].paymentMethod.slice(1),
                                                10
                                            );

                                            const dueDate = new Date(lines[0].docDate);
                                            dueDate.setDate(dueDate.getDate() + days);

                                            return `${dueDate.toLocaleDateString("en-GB")} (${days} Days)`;
                                        })()
                                        : ""
                                }
                                disabled
                    ...
                    <Col md="3" sm="3">
                        <FormGroup>
                            <Label className="font-weight-bold d-block" style={{ textAlign: "left" }}>House Bank / Account ID</Label>
                            <Input type="text" placeholder="Invoice Date" value={simulateData[0]?.HOUSEBANK + ' / ' + simulateData[0]?.ACCOUNT_ID} disabled />
                        </FormGroup>
                    </Col>
                </Row>
                <div style={{ width: '100%', overflowX: 'auto' }}>
                    ...
                    <thead>
                        ...
                        <th className="bg-primary text-white" style={{ width: '15%' }}>HSN Code</th>
                ...
                <Col md="6" sm="6" className="d-flex justify-content-end gap-3">
                    ...
                    <Button.Ripple
                        color="primary"
                        type="button"
                        onClick={() => handleAction((lines[0]?.status == 4 || actualStatus == 4) ? 4 : 3, 2)}
                        className="d-flex align-items-center"
                    >
                        <CheckCircle size={16} className="me-1" />
                        Miro Park
                    ...
                    <Button.Ripple
                        color="success"
                        type="button"
                        onClick={() => handleAction((lines[0]?.status == 4 || actualStatus == 4) ? 4 : 3, 3)}
                        className="d-flex align-items-center"
                    >
