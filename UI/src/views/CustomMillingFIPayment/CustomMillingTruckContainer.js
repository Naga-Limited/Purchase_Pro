...
const CustomMillingTruckContainer = ({ }) => {
    ...
    const [date_control, setDate_control] = useState('');
      ...
      const GetPODetails = () => {
        ...
        apiPostMethod(apiBaseUrl + `SupplierDispatch/getCustomMillingPoNumbersWheat/${UserDetails.USERID}/${fromDateMilliSecond}/${toDateMilliSecond}/${UserDetails.plantids.length > 0 ? UserDetails.plantids.join(',') : 0}/${0}`)
          .then((response) => {
    ...
    const exportVehicleDetailsToExcel = () => {
        const rows = clubbedItems.length > 0 ? clubbedItems : (data && data.TRUCK_NO ? [data] : []);
        if (!rows.length) {
        ...
        const formattedData = rows.map((row) => ({
            ...
            "VA Number": row?.vaNumber || row?.ZVA_NUMBER,
            "PO Number": row?.ZPO_NUMBER,
            "Plant": row?.WERKS,
            "Invoice No": row?.invoice_no,
            "Qty In Ton": row?.gunny_less_wt,
            "Rate": row?.rate,
            "Amount": row?.condition_amount,
        }));
                 ...
                 <Col md="12" sm="12">
                    ...
                    <div style={{ width: "100%", overflowX: "auto", border: "1px solid #ddd" }}>
                        ...
                        >
                            ...
                            <tbody>
                                {(clubbedItems.length > 0 ? clubbedItems : [data]).map((row, i) => (
                                    <tr key={i}>
                                        <td>{row?.TRUCK_NO}</td>
                                        <td>{row?.vaNumber || row?.ZVA_NUMBER}</td>
                                        <td>{row?.ZPO_NUMBER}</td>
                                        <td>{row?.WERKS}</td>
                                        <td>{row?.invoice_no}</td>
                                        <td>{row?.gunny_less_wt}</td>
                                        <td>{row?.rate}</td>
                                        <td>{row?.condition_amount}</td>
                                    </tr>
                                ))}
