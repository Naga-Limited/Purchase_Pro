...
const CrecPaymentInfoDEPMGAPPROVE = ({ title, url, actionRenderer }) => {
    ...
    const form = useFormik({
        ...
        initialValues: { date: defaultDateRange },
    ...
    const loadTableData = async () => {
        ...
        showLoader();
        try {
            const response = await apiPostMethod(apiBaseUrl + "RecurringPaymentController/recpaymentinfoapproval", postdata);
            const { data } = response;
            if (data.success) {
    ...
    const handleApprove = (row) => { setSelectedRow(row); setApproveModalOpen(true); };
    const handleReject = (row) => { setRejectedRow(row); setRejectModalOpen(true); };

    const handleApproveConfirmation = async (row) => {
        const postdata = { id: row.rp_id, remarks: remarks, approved_by: UserDetails.USERID };
        showLoader();
        try {
            const response = await apiPostMethod(apiBaseUrl + "RecurringPaymentController/recpaymentinfoapprovaldepartmentMG", postdata);
            const { data } = response;
            if (data.success) {
            ...
            } else errorToast("Failed to approve Payment item");
        } catch (error) {
            errorToast("Something went wrong, please try again after some time");
        } finally { hideLoader(); }
    };
    ...
    const actionsCol = {
        ...
        cell: (row) => (
            <>
                <Button.Ripple color="primary" onClick={() => handleViewModalOpen(row)}>View</Button.Ripple>&nbsp;
            </>
        ),
    };
    ...
    return (
        <div>
            <Card>
                <CardHeader>
                    <CardTitle>Recurring Payment Info Department MG Approval</CardTitle>
                </CardHeader>
                <CardBody>
