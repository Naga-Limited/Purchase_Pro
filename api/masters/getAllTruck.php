...
}else if($entityVAContent->formType == "Process" && $entityVAContent->ScreenName == 'Quality Check') {
  echo fetchAllProcessTruckArrivalQCCheck($connect, $entityVAContent);
}else if($entityVAContent->formType == "F" && $entityVAContent->ScreenName == 'Quality Check') {
  echo fetchAllProcessTruckArrivalQCCheck($connect, $entityVAContent);
}
...
//    OR  ZPO_NUMBER like '%" . $record->searchTxt . "%'
 ...
 //   or ZVA_NUMBER like '%" . $record->searchTxt . "%' or ZPO_NUMBER like '%" . $record->searchTxt . "%'  or TRUCK_NO like '%" . $record->searchTxt . "%')");
  ...
  $fetchsql =
    ...
    LEFT JOIN  master_storage c ON a.LGORT=c.LGORT and b.id = c.plantid
    where "
    . join(" AND ", $filters) . " order by PI_REFID limit " .
    $record->startCount .
    ",$pageSize";
  if ($Debug == 1) {
    echo $CurrentTime;
...
function fetchAllProcessTruckArrivalQCCheck($connect, $record)
{
  $CurrentTime = date("d-m-Y H:i:s") . "\n";
